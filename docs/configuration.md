...
A persistent `build-dir` keeps Conan's build tree across runs so recompilation
is incremental. The staging area that becomes the wheel is deleted and recreated
on every build, so wheel contents never carry files from a previous build.
...
| `clean-after-wheel` | `[tool.conan-py-build]` | Remove the exported Conan reference from the cache after building the wheel | `true` |
...
1. Deploys them to `build/.conan-libs/` under the project root.
2. Patches the extension RPATH so that repair tools can discover and bundle
   those libraries. On Linux this patching is done with `patchelf
   --add-rpath` during `build_wheel()` itself, before any repair tool runs, so
   `patchelf` must already be available on the build machine (it is
   preinstalled on manylinux images) rather than only at the later
   `auditwheel` step.
...
removes the dependency on the system version. You can inspect which symbols
your extension imports from a given library with `nm -D -u` (Linux),
`nm -u` (macOS), or `dumpbin /imports` (Windows). On Windows, `/exports`
lists symbols the binary provides, not the ones it depends on.
